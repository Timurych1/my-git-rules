...
`git restore -S .` или `git reset .` – отменить изменения индекса по коммиту (откатить индекс до состояния последнего коммита)
`git rm <filename>` – удалить файл из индекса, чтобы перестать хранить его историю